...
def DenseBinaryClassificationNet(input_shape=(8,)):
    ...
    x, marker_id, domain_id = NVTXStart(message='Dense 1',
                                        domain_name='forward',
                                        trainable=True)(x)
    x = Dense(8*128, activation='relu')(x)
    ...
    x, marker_id, domain_id = NVTXStart(message='Dense 3',
                                        domain_name='forward')(x)
    x = Dense(8*64, activation='relu')(x)
...
model.compile(optimizer=opt,
              ...
              metrics=['accuracy'])
model.fit(features, labels, batch_size=8*8, epochs=NUM_EPOCHS,
          callbacks=[nvtx_callback])
print('Using GPU for training with XLA add MixedPrecision took {0} seconds'.format(time.time() - startTime))